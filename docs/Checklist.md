...
- [ ] 消息的特殊属性
  ...
  - [ ] 右键(移动界面是长按)每一条消息，可以出现这多个选项(菜单栏)。
    - [ ] 那就可以对后面两个操作进行批量处理了。
    - [ ] 为了终止LLM持续输出, 可以在上面强制终止
  - [ ] 一个消息能被标记一些属性. 被标记属性的消息也能被取消标记
...
- [ ] 压缩长期记忆
  ...
  - [ ] 自动更新后, 通知舍长
- [ ] 整理聊天记录( 此功能暂时不做,  对上下文的要求太高 )

  - [ ] 彻底异步
  - [ ] 完成后发送消息
  - [ ] 点击整理聊天记录的消息之后, 弹出消息的内容
  - [ ] mini batch, 上下文可以很大, 甚至 1000条
  - [ ] 滚动上下文, 0-100, 80-180, 160-260, 240-340, 320-420, 400-500(当然, 有时候机器人的消息会很长, 那么这个minibatch的上下文窗口就需要很大了)
  - [ ] 特殊的Prompt(携带用户信息/时间戳和时间/机器人的名字)
- [ ] 联网搜索(API调用)
