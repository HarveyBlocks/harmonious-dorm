...
- [x] 如果机器人调用了Tool, 需要把它做了什么都告诉用户, 做了什么, 参数是啥
- [x] AI权限设置(舍长可设置, 且仅能手动设置)(在机器人卡)
- [x] Tool call 的配置写一个文件, tool call 的 function 写入一个文件夹下, 一个funciton对应一个文件
...
- [ ] 压缩长期记忆
  ...
  - [ ] 自动更新后, 通知舍长
- [ ] 整理聊天记录(用户可以设置条数, 10-100)

  - [ ] 独立的机器人请求, 独立设计系统Prompt
    - 需要机器人自己的信息(名字. 人设, 其他内容等, 同一般聊天)
    - 需要宿舍成员信息(成员名等, 同一般聊天)
    - 需要宿舍的信息, 同一般聊天
    - 需要当前时间戳timestamp
    - 需要当前时间yyyy-MM-dd hh:mm:ss
    - 对输出的长度限制, 比聊天的长度限制大!
    - 不需要Tool
    - 提示词不一样, 专注于总结的功能
  - [ ] 彻底异步
