...
async function run() {
  const dorm = await prisma.dorm.findUnique({
    where: { inviteCode: 'D0003' },
    select: { id: true, name: true },
  });
  if (!dorm) {
    throw new Error('邀请码 D0003 不存在');
  }
  ...
  if (billIds.length > 0) {
    await prisma.billParticipant.deleteMany({
      where: { billId: { in: billIds.map((item) => item.id) } },
    });
  }
...
run()
  .then(async () => {
    ...
    console.log('D0003 heavy seed done');
  })
  .catch(async (error) => {
    console.error(error);
    await prisma.$disconnect();
