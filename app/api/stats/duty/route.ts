...
export async function GET(request: Request) {
  return withApiGuard(async () => {
    ...
    const periodType = (searchParams.get('periodType') || 'month') as 'month' | 'quarter' | 'year';
    ...
    const payload = await getDutyStats(session, { periodType, year, marker, lineGranularity });
