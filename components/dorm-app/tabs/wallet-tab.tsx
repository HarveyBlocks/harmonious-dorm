...
export function WalletTab(props: {
  ...
  billUnpaidListRef: React.RefObject<HTMLDivElement>;
  billPaidListRef: React.RefObject<HTMLDivElement>;
  onBillUnpaidListScroll: (event: React.UIEvent<HTMLDivElement>) => void;
  onBillPaidListScroll: (event: React.UIEvent<HTMLDivElement>) => void;
  ...
  setParticipantWeights: React.Dispatch<React.SetStateAction<Record<number, string>>>;
  ...
  const buildBillLine = (bill: any) => {
    const categoryText = bill.customCategory || categoryLabel(p.me?.language || 'zh-CN', bill.category);
    return categoryText;
  };
  ...
  return (
    <motion.div key="wallet" animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <QuickBillForm
        ...
        billWeightLabel={p.pText.billWeight}
